...
# Initialize YOLOv8 model and webcam
...
def get_gesture():
    """Capture and process webcam frame to detect hand gestures"""
    ...
    if len(results) > 0 and len(results[0].boxes) > 0:
        ...
        # Map class_id to direction - FINAL CORRECTED MAPPING
        class_mapping = {
            3: 'up',     # Changed to 3
            0: 'down',   # Stays 0
            2: 'right',  # Stays 2
        ...
        if conf > 0.5:  # Confidence threshold
            detected_gesture = class_mapping.get(class_id)
            print(f"Detected gesture: {detected_gesture} (class_id: {class_id})")  # Debug print
...
def move_player():
    ...
    # Get gesture from computer vision
    gesture = get_gesture()
    
    # Convert gesture to movement
    if gesture == 'right':
        moving_right = True
        moving_left = False
        player_img = p_right
